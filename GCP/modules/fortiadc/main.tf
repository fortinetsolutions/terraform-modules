# Create log disk for FortiADC
resource "google_compute_disk" "fadc_logdisk" {
...
resource "google_compute_instance" "fadc_instance" {
  name           = "${var.name}-fadc"
  ...
  attached_disk {
    source = google_compute_disk.fadc_logdisk.name
  }
  ...
  # Metadata which will deploy the license and configure
  metadata = {
    user-data = data.template_file.setup-fadc-instance.rendered
